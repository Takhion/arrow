...
object MonadFilterLaws {
  ...
  inline fun <F: KindType> laws(MF: MonadFilter<F>, noinline cf: (Int) -> Kind<F, Int>, EQ: Eq<Kind<F, Int>>): List<Law> =
  ...
  fun <F: KindType> MonadFilter<F>.monadFilterLeftEmpty(EQ: Eq<Kind<F, Int>>): Unit =
    forAll(genFunctionAToB(genApplicative(Gen.int(), this)), { f: (Int) -> Kind<F, Int> ->
      empty<Int>().flatMap(f).equalUnderTheLaw(empty(), EQ)
    })

  fun <F: KindType> MonadFilter<F>.monadFilterRightEmpty(EQ: Eq<Kind<F, Int>>): Unit =
    forAll(genApplicative(Gen.int(), this), { fa: Kind<F, Int> ->
      fa.flatMap({ empty<Int>() }).equalUnderTheLaw(empty(), EQ)
    })

  fun <F: KindType> MonadFilter<F>.monadFilterConsistency(cf: (Int) -> Kind<F, Int>, EQ: Eq<Kind<F, Int>>): Unit =
  ...
  fun <F: KindType> MonadFilter<F>.monadFilterEmptyComprehensions(EQ: Eq<Kind<F, Int>>): Unit =
  ...
  fun <F: KindType> MonadFilter<F>.monadFilterBindWithFilterComprehensions(EQ: Eq<Kind<F, Int>>): Unit =
    forAll(Gen.bool(), Gen.int(), { guard: Boolean, n: Int ->
      bindingFilter {
