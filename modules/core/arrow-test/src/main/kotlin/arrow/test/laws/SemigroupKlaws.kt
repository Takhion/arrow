...
object SemigroupKLaws {
  ...
  inline fun <F: KindType> laws(SGK: SemigroupK<F>, AP: Applicative<F>, EQ: Eq<Kind<F, Int>>): List<Law> =
    listOf(Law("SemigroupK: associativity", { SGK.semigroupKAssociative(AP::just, EQ) }))

  inline fun <F: KindType> laws(SGK: SemigroupK<F>, noinline f: (Int) -> Kind<F, Int>, EQ: Eq<Kind<F, Int>>): List<Law> =
    listOf(Law("SemigroupK: associativity", { SGK.semigroupKAssociative(f, EQ) }))

  fun <F: KindType> SemigroupK<F>.semigroupKAssociative(f: (Int) -> Kind<F, Int>, EQ: Eq<Kind<F, Int>>): Unit =
