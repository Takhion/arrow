...
object FunctorLaws {
  ...
  inline fun <F: KindType> laws(AP: Applicative<F>, EQ: Eq<Kind<F, Int>>): List<Law> =
    listOf(
  ...
  fun <F: KindType> laws(FF: Functor<F>, f: (Int) -> Kind<F, Int>, EQ: Eq<Kind<F, Int>>): List<Law> =
    listOf(
  ...
  fun <F: KindType> Functor<F>.covariantIdentity(f: (Int) -> Kind<F, Int>, EQ: Eq<Kind<F, Int>>): Unit =
    forAll(genConstructor(Gen.int(), f), { fa: Kind<F, Int> ->
      fa.map(::identity).equalUnderTheLaw(fa, EQ)
    })

  fun <F: KindType> Functor<F>.covariantComposition(ff: (Int) -> Kind<F, Int>, EQ: Eq<Kind<F, Int>>): Unit =
    forAll(
