...
object TraverseFilterLaws {
  ...
  inline fun <F: KindType> laws(TF: TraverseFilter<F>, GA: Applicative<F>, noinline cf: (Int) -> Kind<F, Int>, EQ: Eq<Kind<F, Int>>, EQ_NESTED: Eq<Kind<F, Kind<F, Int>>> = Eq.any()): List<Law> =
  ...
  fun <F: KindType> TraverseFilter<F>.identityTraverseFilter(GA: Applicative<F>, EQ: Eq<Kind<F, Kind<F, Int>>> = Eq.any()) =
    forAll(genApplicative(genIntSmall(), GA), { fa: Kind<F, Int> ->
      fa.traverseFilter(GA, { GA.just(Some(it)) }).equalUnderTheLaw(GA.just(fa), EQ)
    })

  fun <F: KindType> TraverseFilter<F>.filterAconsistentWithTraverseFilter(GA: Applicative<F>, EQ: Eq<Kind<F, Kind<F, Int>>> = Eq.any()) =
