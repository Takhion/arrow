...
@higherkind
data class Coproduct<F: KindType, G: KindType, A>(val run: Either<Kind<F, A>, Kind<G, A>>) : CoproductOf<F, G, A>(), CoproductKindedJ<F, G, A> {
  ...
  fun <H: KindType> fold(f: FunctionK<F, H>, g: FunctionK<G, H>): Kind<H, A> =
    run.fold({ f(it) }, { g(it) })
  ...
  fun <H: KindType, B> traverse(GA: Applicative<H>, FT: Traverse<F>, GT: Traverse<G>, f: (A) -> Kind<H, B>): Kind<H, Coproduct<F, G, B>> = GA.run {
    run.fold({
  ...
  companion object {
    operator fun <F: KindType, G: KindType, A> invoke(run: Either<Kind<F, A>, Kind<G, A>>): Coproduct<F, G, A> =
      Coproduct(run)
  }
}

fun <F: KindType, G: KindType, A, H: KindType> CoproductOf<F, G, Kind<H, A>>.sequence(HA: Applicative<H>, FT: Traverse<F>, GT: Traverse<G>): Kind<H, Coproduct<F, G, A>> =
  fix().traverse(HA, FT, GT, ::identity)

fun <F: KindType, G: KindType, A> EitherOf<Kind<F, A>, Kind<G, A>>.coproduct(): Coproduct<F, G, A> =
  Coproduct(fix())

fun <F: KindType, G: KindType, H: KindType> FunctionK<F, G>.or(h: FunctionK<H, G>): FunctionK<CoproductPartialOf<F, H>, G> =
